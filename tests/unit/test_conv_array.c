...
static void test_invalid_img_num(void **state) {
    assert_int_equal(
        lcl_conv_array(NULL, NULL, NULL, NULL, -1, (thread_jobs_t){0}),
        LCL_INVALID_ARGUMENT);
    assert_int_equal(lcl_conv_array(NULL, NULL, NULL, NULL, MAX_IMG_NUM + 1,
                                    (thread_jobs_t){0}),
                     LCL_INVALID_ARGUMENT);
}
...
static void test_nullptrs(void **state) {
    ...
    assert_int_equal(
        lcl_conv_array(NULL, targ_paths, modes, filters, 2, (thread_jobs_t){0}),
        LCL_INVALID_ARGUMENT);
    assert_int_equal(
        lcl_conv_array(src_paths, NULL, modes, filters, 2, (thread_jobs_t){0}),
        LCL_INVALID_ARGUMENT);
    assert_int_equal(lcl_conv_array(src_paths, targ_paths, NULL, filters, 2,
                                    (thread_jobs_t){0}),
                     LCL_INVALID_ARGUMENT);
    assert_int_equal(lcl_conv_array(src_paths, targ_paths, modes, NULL, 2,
                                    (thread_jobs_t){0}),
                     LCL_INVALID_ARGUMENT);
}
...
static void test_lcl_conv_array1(void **state) {
    ...
    assert_int_equal(
        lcl_conv_array(src_paths, targ_paths, modes, filters, 1, jobs), LCL_OK);
...
static void test_lcl_conv_array2(void **state) {
    ...
    assert_int_equal(
        lcl_conv_array(src_paths, targ_paths, modes, filters, 2, jobs), LCL_OK);
