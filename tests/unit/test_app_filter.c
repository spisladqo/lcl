...
static void test_nullptrs(void **state) {
    ...
    assert_int_equal(lcl_app_filter(pilewise, 1, NULL, NULL, NULL), LCL_INVALID_ARGUMENT);
    assert_int_equal(lcl_app_filter(pilewise, 1, NULL, &src, &targ), LCL_INVALID_ARGUMENT);
    assert_int_equal(lcl_app_filter(pilewise, 1, &filter, NULL, &targ), LCL_INVALID_ARGUMENT);
    assert_int_equal(lcl_app_filter(pilewise, 1, &filter, &src, NULL), LCL_INVALID_ARGUMENT);
}
...
static void test_diff_sizes(void **state) {
    ...
    assert_int_equal(lcl_app_filter(pilewise, 1, &filter, &src, &targ), LCL_SRC_TARG_DIFF_SIZES);
}
...
static void test_too_many_threads(void **state) {
    ...
    assert_int_equal(lcl_app_filter(pilewise, MAX_THREADS + 1, &filter, &src, &targ), LCL_INVALID_ARGUMENT);
}
...
static void test_edges(void **state) {
    ...
    for (int y = 0; y < 2; y++) {
        for (int x = 0; x < 2; x++) {
            assert_true(targ->img_pixels[y][x].red >= 0 && targ->img_pixels[y][x].red <= 255);
            assert_true(targ->img_pixels[y][x].green >= 0 && targ->img_pixels[y][x].green <= 255);
            assert_true(targ->img_pixels[y][x].blue >= 0 && targ->img_pixels[y][x].blue <= 255);
        }
    }
...
static void test_diff_modes(void **state) {
    ...
    for (int y = 0; y < 3; y++) {
        for (int x = 0; x < 3; x++) {
            assert_int_equal(targ1->img_pixels[y][x].red, targ2->img_pixels[y][x].red);
            assert_int_equal(targ1->img_pixels[y][x].green, targ2->img_pixels[y][x].green);
            assert_int_equal(targ1->img_pixels[y][x].blue, targ2->img_pixels[y][x].blue);
            
            assert_int_equal(targ2->img_pixels[y][x].red, targ3->img_pixels[y][x].red);
            assert_int_equal(targ2->img_pixels[y][x].green, targ3->img_pixels[y][x].green);
            assert_int_equal(targ2->img_pixels[y][x].blue, targ3->img_pixels[y][x].blue);
        }
    }
