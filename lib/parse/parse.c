...
void print_help(const char* name) {
    printf("Usage: %s --mode=<mode> --src=<source> --target=<target> --filter=<filter> [options]\n\n", name);
    ...
    printf("                          id, blur, Blur, BLUR, edge, sharp, negative, emboss, etc.\n\n");
    printf("Required Arguments if mode is par:\n");
    printf("  --conv=<type>           Convolution type (pixelwise, pilewise, rowwise, columnwise)\n");
    printf("  --nthreads=<n>          Number of threads (default: 1)\n");
    printf("Optional arguments:\n");
    printf("  --help                  Show this help message\n\n");
    printf("Examples:\n");
    printf("  %s --mode=seq --src=input.bmp --target=output.bmp --filter=blur\n", name);
    printf("  %s --mode=par --src=in.bmp --target=out.bmp --filter=edge --nthreads=4 --conv=pixelwise\n", name);
}
...
int lcl_parse(int argc, char* argv[], lcl_parse_arg* args) {
    ...
    while ((opt = getopt_long(argc, argv, "m:s:t:f:c:n:", long_options, &option_index)) != -1) {
        switch (opt) {
            case 'h':
                print_help(argv[0]);
                return 0;
            ...
            case 'f':
                if (strcmp(optarg, "id") == 0) {
                        ...
                        fprintf(stderr, "error: Unknown filter '%s'. Please check lib/conv/common.h to find the list of filters to use. For example, --filter=id).\n", optarg);
                        return -1;
                    }
                break;
            ...
            case 'c':
                 if (strcmp(optarg, "pixelwise") == 0) {
                    ...
                    printf("error: invalid conv '%s'. Must be one of the following: pixelwise, pilewise, rowwise, columnwise\n", optarg);
                    return INVALID_VAL;
                 }
                break;
            case 'n':
